...
describe('processTemplate', () => {
  it('replaces <%= keyName %> in source if provided as data', async () => {
    expect(processTemplate('prefix <%= name %> suffix', {name : 'foo'}))
        .to.equal(
            'prefix foo suffix',
        );
  });

  it('replaces multiple instances <%= keyName %> in source if provided as data',
     async () => {
       expect(processTemplate('prefix <%= name %> suffix <%= name %>',
                              {name : 'foo'}))
           .to.equal(
  ...
  it('ignores <%= keyName %> in source if not provided as data', async () => {
    expect(processTemplate('prefix <%= name %> suffix', {foo : 'foo'}))
        .to.equal(
            'prefix <%= name %> suffix',
        );
  });
...
describe('writeFileToPath', () => {
  ...
  it('will override content for the same path', async () => {
    ...
    expect(virtualFiles).to.deep.equal([
      {path : 'foo/bar.js', content : 'updated barfile'}
    ]);
  });
});
...
describe('writeFileToPathOnDisk', () => {
  beforeEach(() => { fs.writeFileSync(`./__tmpfoo.txt`, 'content of foo'); });
  afterEach(() => {
    if (fs.existsSync(`./__tmpfoo.txt`)) {
      fs.unlinkSync(`./__tmpfoo.txt`);
    }
  ...
  it('will not override by default', async () => {
    await writeFileToPathOnDisk(`./__tmpfoo.txt`, 'updatedfoofile',
                                {ask : false});
    expect(fs.readFileSync(`./__tmpfoo.txt`, 'utf-8'))
        .to.equal('content of foo');
  });
...
describe('copyTemplates', () => {
  it('returns a promise which resolves with the copied and processed files',
     async () => {
       const copiedFiles =
           await copyTemplates(`./test/template/**/*`, `source`, {
             name : 'hello-world',
           });
...
describe('copyTemplateJsonInto', () => {
  ...
  it('merges objects', async () => {
    ...
    deleteVirtualFile(
        'generator/package.json'); // just used to make test easier
...
describe('executeMixinGenerator', () => {
  it('combines multiple mixins and executes them', async () => {
    const FooMixin = subclass => class extends subclass {
      constructor() {
        super();
        this.foo = true;
      }
    ...
    const BarMixin = subclass => class extends subclass {
      constructor() {
        super();
        this.bar = true;
      }
...
describe('optionsToCommand', () => {
  it('supports strings', async () => {
    ...
    expect(optionsToCommand(options))
        .to.equal('npm init @open-wc --type scaffold ');
  });
  ...
  it('supports boolean', async () => {
    ...
    expect(optionsToCommand(options))
        .to.equal('npm init @open-wc --writeToDisk ');
    const options2 = {
      writeToDisk : false,
    };
  ...
  it('supports arrays', async () => {
    ...
    expect(optionsToCommand(options))
        .to.equal('npm init @open-wc --features testing demoing ');
  });
...
describe('filesToTree', () => {
  it('renders a single file', async () => {
    expect(filesToTree([ './foo.js' ])).to.equal([
      './', '└── foo.js\n'
    ].join('\n'));
  });
