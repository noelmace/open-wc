...
const safeReduce = (f, initial) => xs =>
    (Array.isArray(xs) ? xs.reduce(f, initial) : xs);
...
function featureReadme(acc, feature, i, a) {
  return `${acc + featureReadmeBlurb(feature)}${
      i === a.length - 1 ? '' : '\n'}`;
}
...
export const WcLitElementMixin = subclass => class extends subclass {
  async execute() {
    ...
    this.copyTemplate(`${__dirname}/templates/_my-el.js`,
...
export const WcLitElementPackageMixin = subclass =>
    class extends CommonRepoMixin
(WcLitElementMixin(subclass)) {
  async execute() {
    await super.execute();
