 ...
 *
 * example:
 * npm init @open-wc --type scaffold --scaffoldType app --tagName foo-bar
 * --installDependencies false npm init @open-wc --type upgrade --features
 * linting demoing --tagName foo-bar --scaffoldFilesFor demoing
...
export const AppMixin = subclass =>
  ...
  async execute() {
    ...
    const questions = [
      ...
      {
        type : (prev, all) => all.scaffoldType === 'wc' ||
        ...
        choices : (
            prev,
            all) => [{title : 'Linting (eslint & prettier)', value : 'linting'},
