...
class FeatureA extends LitElement {
  render() { return html` <div>Element A</div> `; }
}
...
describe('ScopedElementsMixin', () => {
  it('has a default value for "static get scopedElements()" of {}',
     async () => {
  ...
  it('supports the "same" tag name in the template for multiple different sub components',
     async () => {
       ...
       class PageA extends ScopedElementsMixin
       (LitElement) {
         static get scopedElements() {
           return {
         ...
         render() { return html` <feature-a></feature-a> `; }
       }

       class PageB extends ScopedElementsMixin
       (LitElement) {
         static get scopedElements() {
           return {
       ...
       const tag = defineCE(
           class extends ScopedElementsMixin(LitElement) {
             ...
             render() { return html` <page-a></page-a><page-b></page-b> `; }
           },
       );
       const el = await fixture(`<${tag}></${tag}>`);
       ...
       const pageAFeatureANode =
           el.shadowRoot.children[0].shadowRoot.children[0];
       const pageBFeatureANode =
           el.shadowRoot.children[1].shadowRoot.children[0];

       expect(pageAFeatureANode.tagName.toLowerCase())
           .to.match(new RegExp(`feature-a-\\d{1,5}`));
       expect(pageAFeatureANode).to.be.an.instanceOf(FeatureA1x);
       expect(pageBFeatureANode.tagName.toLowerCase())
           .to.match(new RegExp(`feature-a-\\d{1,5}`));
       expect(pageBFeatureANode).to.be.an.instanceOf(FeatureA2x);
     });

  it('supports to use a shared template and use it with different sub components',
     async () => {
  ...
  it('supports to extend as ScopedElements component without defining unused sub components',
     async () => {
       class FeatureC extends LitElement {
         render() { return html` <div>Element C</div> `; }
       }

       class FeatureD extends LitElement {
         render() { return html` <div>Element D</div> `; }
       }
       ...
       const tag = defineCE(
           class extends ScopedElementsMixin(PageA) {
             render() { return html` <feature-c></feature-c> `; }
           },
       );
       const el = await fixture(`<${tag}></${tag}>`);
  ...
  it("support define a lazy element even if it's not used in previous templates",
     async () => {
       class LazyElement extends LitElement {
         render() { return html` <div>Lazy element</div> `; }
       }
  ...
  it('should reuse the global tag if defined with the same name and class reference',
     async () => {
       class ItemA extends LitElement {
         render() { return html` <div>Item A</div> `; }
       }
       ...
       const tag = defineCE(
           class ContainerElement extends ScopedElementsMixin(LitElement) {
             ...
             render() { return html` <item-a></item-a> `; }
           },
       );
  ...
  describe('getScopedTagName', () => {
    it('should return the scoped tag name for a registered element',
       async () => {
         const chars = `-|\\.|[0-9]|[a-z]`;
         const tagRegExp =
             new RegExp(`[a-z](${chars})*-(${chars})*-[0-9]{1,5}`);
         ...
         expect(el.constructor.getScopedTagName('feature-a'))
             .to.match(tagRegExp);
         expect(el.constructor.getScopedTagName('feature-b'))
             .to.match(tagRegExp);
       });

    it('should return the scoped tag name for a non already registered element',
       async () => {
         const chars = `-|\\.|[0-9]|[a-z]`;
         const tagRegExp =
             new RegExp(`[a-z](${chars})*-(${chars})*-[0-9]{1,5}`);
         ...
         class UnregisteredFeature extends LitElement {
           render() { return html` <div>Unregistered feature</div> `; }
         }
         ...
         expect(el.constructor.getScopedTagName('unregistered-feature'))
             .to.match(tagRegExp);
       });
  });
