...
function createPolyfillsData(cfg) {
  ...
  if (polyfills.dynamicImport) {
    addPolyfillConfig({
       ...
       * a browser which doesn't support it throws a syntax error and prevents
       * the entire script from being run, so we need to dynamically create and
       * execute a function and catch the error.
       *
       * CSP can block the dynamic function, in which case the polyfill will
       * always be loaded which is ok. The polyfill itself uses Blob, which
  ...
  if (polyfills.webcomponents && polyfills.shadyCssCustomStyle) {
    // shadycss/custom-style-interface polyfill *must* load after the
    // webcomponents polyfill or it doesn't work. to get around that, concat the
    // two together.
