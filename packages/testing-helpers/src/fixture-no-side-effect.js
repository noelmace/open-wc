 ...
 * @property {Element} [parentNode] optional parent node to render the fixture's
 * template to
 * @property {import('@open-wc/scoped-elements').ScopedElementsMap}
 * [scopedElements] optional scoped-elements definition map
 */
 ...
 *
 * @template {Element} T
 * @param {import('./litFixture').LitHTMLRenderable} template Either a string or
 ...
 * `el.componentOnReady()`;
 *
 * If none of those specfic Promise hooks are found, it will wait for one frame
 * via `await nextFrame()`.
 *
 ...
 *     lit-html TemplateResult
 * @param {FixtureOptions} [options]
 * @returns {Promise<T>} A Promise that will resolve to the first child of the
 *     rendered DOM
 */
