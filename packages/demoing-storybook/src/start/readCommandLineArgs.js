...
module.exports = function readCommandLineArgs() {
  ...
  const mainFilePath =
      path.join(process.cwd(), tmpConfig['config-dir'], 'main.js');
  ...
  if (fs.existsSync(mainFilePath)) {
    ...
    if (mainJs.stories) {
      mainJs.stories = typeof mainJs.stories === 'string' ? [ mainJs.stories ]
                                                          : mainJs.stories;
  ...
  const cliOptions = [
    ...
    },
    {
      ...
      description :
          'List of story files e.g. --stories stories/*.stories.\\{js,mdx\\}',
    },
    {name : 'help', type : Boolean, description : 'See all options'},
  ];

  const storybookArgs = commandLineArgs(cliOptions, {partial : true});
  storybookArgs.stories = typeof storybookArgs.stories === 'string'
  ...
  const esDevServerConfig =
      esDevServerCommandLineArgs(storybookArgs._unknown || [], {
