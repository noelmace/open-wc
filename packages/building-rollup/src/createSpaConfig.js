...
/**
 * @typedef {import('polyfills-loader').PolyfillsLoaderConfig}
...
const {pluginWithOptions, applyServiceWorkerRegistration, isFalsy} =
    require('./utils');
...
function createSpaConfig(options) {
  ...
  if (userOptions.legacyBuild) {
    ...
    polyfillsLoaderConfig = {
      ...
      legacyOutput : {
        ...
        type : 'systemjs',
        test :
            // test if browser supports dynamic imports (and thus modules).
            // import.meta.url cannot be tested
  ...
  return merge(basicConfig, {
    plugins : [
      ...
      pluginWithOptions(polyfillsLoader, userOptions.polyfillsLoader,
                        polyfillsLoaderConfig),
